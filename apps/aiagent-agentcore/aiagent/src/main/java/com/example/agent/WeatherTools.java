...
class WeatherTools {
	...
	// Use JDK HttpClient instead of Netty to ensure system DNS resolver is used
	// (Netty's async DNS resolver may bypass VPC DNS in containerized environments)
	...
	@Tool(description = """
			...
			""")
	public String getWeather(@ToolParam(description = "City name, e.g. Paris, London, New York") String city,
		...
		try {
			...
			var weather = restClient.get()
				.uri("https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={lon}"
						...
						+ "&start_date={startDate}&end_date={endDate}", loc.get("latitude"), loc.get("longitude"), date,
						date)
				.retrieve()
				.body(MAP_TYPE);
			...
			var result = "Weather for %s on %s: Min: %.1f%s, Max: %.1f%s".formatted(loc.get("name"), date,
			...
			log.error("getWeather error: {}", e.getMessage(), e);
