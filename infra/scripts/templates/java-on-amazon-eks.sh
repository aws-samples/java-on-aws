...
# ---------------------------------------------------------------------------
# STARTING POINT ENDS HERE. Everything below is run manually on the IDE for now
# and uncommented once validated. All of it runs as ec2-user with the IDE role.
# ---------------------------------------------------------------------------
...
# labs; --to is exclusive so ECS (page 140) is skipped. Leaves the Dockerfile and
# k8s/*.yaml in ~/environment/unicorn-store-spring and the app live on EKS.
...
# apply the sidecar-injection MutatingPolicy). Separate script (Phase B).
...
# Phase 8: Optimization agent (build/push image, create Bedrock KB via IDE role using
# the CDK-provisioned exec role, deploy perf-optimizer MCP server). Separate script (Phase B).
