...
cat > "${DASH}" <<DASH_EOF
{
  ...
  "timezone": "browser", "schemaVersion": 39, "refresh": "15s",
  "time": { "from": "now-1h", "to": "now" }, "templating": { "list": [] },
  "panels": [
    ...
    { "type": "stat", "id": 1, "title": "Startup (ready time)", "gridPos": {"x":0,"y":1,"w":5,"h":4},
      ...
      "targets": [{"refId":"A","datasource":{"type":"prometheus","uid":"promds"},
        "expr":"max(application_ready_time_seconds{namespace=\"${APP_NS}\"})"}] },
    { "type": "stat", "id": 2, "title": "p99 request latency", "gridPos": {"x":5,"y":1,"w":5,"h":4},
      "datasource": {"type":"prometheus","uid":"promds"}, "fieldConfig": {"defaults":{"unit":"s"}},
      "targets": [{"refId":"A","datasource":{"type":"prometheus","uid":"promds"},
        "expr":"histogram_quantile(0.99, sum(rate(http_server_requests_seconds_bucket{namespace=\"${APP_NS}\"}[5m])) by (le))"}] },
    ...
    { "type": "timeseries", "id": 7, "title": "Request latency (p50 / p99)", "gridPos": {"x":12,"y":6,"w":12,"h":8},
      "datasource": {"type":"prometheus","uid":"promds"}, "fieldConfig": {"defaults":{"unit":"s"}},
      "targets": [
        {"refId":"A","datasource":{"type":"prometheus","uid":"promds"},
         "expr":"histogram_quantile(0.50, sum(rate(http_server_requests_seconds_bucket{namespace=\"${APP_NS}\"}[5m])) by (le))","legendFormat":"p50"},
        {"refId":"B","datasource":{"type":"prometheus","uid":"promds"},
         "expr":"histogram_quantile(0.99, sum(rate(http_server_requests_seconds_bucket{namespace=\"${APP_NS}\"}[5m])) by (le))","legendFormat":"p99"}] },
    { "type": "timeseries", "id": 8, "title": "Startup per pod (ready time)", "gridPos": {"x":0,"y":14,"w":12,"h":8},
      "datasource": {"type":"prometheus","uid":"promds"}, "fieldConfig": {"defaults":{"unit":"s"}},
      "targets": [{"refId":"A","datasource":{"type":"prometheus","uid":"promds"},
        "expr":"application_ready_time_seconds{namespace=\"${APP_NS}\"}","legendFormat":"{{pod}}"}] },
